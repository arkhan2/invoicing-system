...
import { getVendorsList } from "./actions";
import { VendorsDataProvider } from "./VendorsDataContext";
import { VendorsViewSwitcher } from "./VendorsViewSwitcher";
import type { Vendor } from "./VendorForm";

const PER_PAGE_OPTIONS = [50, 100, 200] as const;

export function VendorsDataLoader({
  companyId,
  children,
}: {
  companyId: string;
  ...
  useEffect(() => {
    let cancelled = false;
    getVendorsList(companyId, page, perPage, searchQuery || undefined).then((res) => {
      if (cancelled) return;
      const vendors: Vendor[] = res.list.map((c) => ({
        id: c.id,
        name: c.name,
      ...
      setData({ totalCount: res.totalCount, list: vendors });
    });
    return () => {
      cancelled = true;
    };
  ...
  return (
    ...
    >
      <VendorsViewSwitcher>{children}</VendorsViewSwitcher>
    </VendorsDataProvider>
  );
}
