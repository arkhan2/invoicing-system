...
import { VendorList, type VendorListRef } from "./VendorList";
...
export function VendorSpreadsheetView({
  ...
  const listRef = useRef<VendorListRef>(null);
    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      <div className={topBarClass}>
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2">
          ...
          <Link
            href={`/dashboard/vendors/import?${new URLSearchParams({
              page: String(page ?? 1),
          ...
          <Link
            href={`/dashboard/vendors?${new URLSearchParams({
              page: String(page ?? 1),
              perPage: String(perPage ?? 100),
              ...(searchQueryProp?.trim() && { q: searchQueryProp.trim() }),
            }).toString()}`}
      ...
      <div className="min-h-0 flex-1 overflow-hidden flex flex-col">
        <div className="card rounded-none border-0 h-full min-h-0 flex flex-col overflow-hidden p-0">
          <VendorList
            ...
            scrollToVendorId={highlightId ?? ((searchQueryProp ?? "").trim() && vendors[0]?.id) ?? undefined}
          />
        </div>
      </div>
      {hasPagination && (
