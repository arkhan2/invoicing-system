...
import { importVendorsFromCsv, type ImportVendorsResult } from "./actions";
...
export function VendorImportPage({ companyId }: { companyId: string }) {
  ...
  const handleImport = async () => {
    ...
    setResult(null);
    setImportProgress(0);
    startGlobalProcessing("Importing vendors…");
    ...
    try {
      for (let i = 0; i < chunks.length; i++) {
        const res = await importVendorsFromCsv(companyId, chunks[i]!, mapping, { extractNtn });
        if (res.error) {
          setResult(res);
      ...
      endGlobalProcessing({ success: `Imported ${totalImported} vendors.` });
      if (totalSkipped > 0) {
    ...
    <div className="flex h-full min-h-0 w-full flex-col">
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-auto">
        <p className="flex-shrink-0 px-4 py-2 text-sm text-[var(--color-on-surface-variant)]">
          {step === "upload" && "Upload a CSV file (e.g. from Zoho). You will map columns to vendor fields in the next step."}
          {step === "mapping" && "Map each vendor field to a CSV column. Rows without a name are skipped."}
          {step === "preview" && `${validRows.length} vendor${validRows.length !== 1 ? "s" : ""} will be imported. ${rows.length - validRows.length} row(s) skipped (no name).`}
        </p>
