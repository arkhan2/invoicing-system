...
const STORAGE_KEY = "customers-search";
...
function loadFromStorage(): CustomersSearchState {
  if (typeof window === "undefined") return {};
  try {
    const raw = sessionStorage.getItem(STORAGE_KEY);
    if (!raw) return {};
    const parsed = JSON.parse(raw) as CustomersSearchState;
    return {
...
function saveToStorage(state: CustomersSearchState) {
  if (typeof window === "undefined") return;
  try {
    const toSave = {
...
/** Build customers list URL with search state. Uses URL params first, then sessionStorage. */
...
/** Build customer detail URL with search state preserved. */
export function customerDetailHref(
  ...
  return `/dashboard/customers/${id}${s ? `?${s}` : ""}`;
}

/** Build customer edit URL with search state preserved. */
export function customerEditHref(
  id: string,
  params: { from?: string; q?: string; page?: number; perPage?: number } = {}
): string {
  const stored = loadFromStorage();
  ...
  if (params.from) searchParams.set("from", params.from);
  const s = searchParams.toString();
  return `/dashboard/customers/${id}/edit${s ? `?${s}` : ""}`;
}

export function useCustomersSearchState() {
