...
import {
  importEstimatesFromCsv,
  type ImportEstimatesResult,
  type MappedEstimate,
  type MappedEstimateItem,
} from "./actions";
import { useEstimatesTopBar } from "./EstimatesTopBarContext";

type ImportResultWithUnlinked = ImportEstimatesResult & { skippedUnlinkedCustomerNames?: string[] };
...
const ESTIMATE_FIELDS: { key: EstimateMappingKey; label: string; level: "Estimate" }[] = [
  { key: "quote_identifier", label: "Quote identifier (group by)", level: "Estimate" },
  { key: "estimate_date", label: "Quote Date", level: "Estimate" },
  { key: "estimate_number", label: "Quote Number", level: "Estimate" },
  { key: "status", label: "Quote Status", level: "Estimate" },
  { key: "customer_name", label: "Customer Name (required for match)", level: "Estimate" },
  { key: "valid_until", label: "Expiry Date", level: "Estimate" },
  { key: "notes", label: "Notes", level: "Estimate" },
  { key: "payment_terms", label: "Terms & Conditions", level: "Estimate" },
  { key: "subject", label: "Subject", level: "Estimate" },
  { key: "project_name", label: "Project Name", level: "Estimate" },
  { key: "total_amount", label: "Total", level: "Estimate" },
  { key: "total_tax", label: "SubTotal (for tax calc)", level: "Estimate" },
  { key: "discount_amount", label: "Entity Discount Amount / Percent", level: "Estimate" },
  { key: "discount_type", label: "Discount Type", level: "Estimate" },
];

const ITEM_FIELDS: { key: EstimateMappingKey; label: string; level: "Line item" }[] = [
...
export type EstimateColumnMapping = Record<EstimateMappingKey, string>;

function buildDefaultMapping(headers: string[]): EstimateColumnMapping {
  ...
  return {
    quote_identifier: pick("Quote ID", "Quote Number", "QuoteId") || pick("Quote Number") || headers[0] || "",
    estimate_date: pick("Quote Date", "Date"),
    estimate_number: pick("Quote Number", "Quote ID"),
    status: pick("Quote Status", "Status"),
    customer_name: pick("Customer Name", "Customer", "CustomerName"),
    valid_until: pick("Expiry Date", "Valid Until"),
    notes: pick("Notes"),
    payment_terms: pick("Terms & Conditions", "Terms and Conditions", "Payment Terms"),
    subject: pick("Subject"),
    project_name: pick("Project Name", "Project"),
    total_amount: pick("Total"),
    total_tax: pick("SubTotal", "Sub Total"),
    discount_amount: pick("Entity Discount Amount", "Entity Discount Percent", "Discount"),
    ...
    sales_tax_applicable: pick("Item Tax Amount", "Tax Amount"),
    discount: pick("Discount Amount", "Item Discount"),
...
export function EstimateImportPage({
  ...
  const [mapping, setMapping] = useState<EstimateColumnMapping>({} as EstimateColumnMapping);
  ...
  const grouped = useMemo(() => {
    const quoteCol = mapping.quote_identifier;
    if (!quoteCol) return [];
    const groups = new Map<string, Record<string, string>[]>();
    for (const row of rows) {
      const key = (row[quoteCol] ?? "").trim() || "__blank__";
      if (!groups.has(key)) groups.set(key, []);
      groups.get(key)!.push(row);
    }
    return Array.from(groups.entries()).map(([key, groupRows]) => ({
      quoteKey: key === "__blank__" ? "" : key,
      rows: groupRows,
    }));
  }, [rows, mapping.quote_identifier]);

  const mappedEstimates: MappedEstimate[] = useMemo(() => {
    const out: MappedEstimate[] = [];
    const g = (row: Record<string, string>, k: EstimateMappingKey) => {
      const col = mapping[k];
      return (col && row[col] ? String(row[col]).trim() : "") as string;
    };
    for (const { quoteKey, rows: groupRows } of grouped) {
      if (!quoteKey || groupRows.length === 0) continue;
      ...
      const items: MappedEstimateItem[] = groupRows
        .map((row) => {
        ...
        .filter((x): x is MappedEstimateItem => x !== null);
      if (items.length === 0) continue;
      out.push({
        customer_name: g(first, "customer_name"),
        estimate_number: g(first, "estimate_number") || quoteKey,
        estimate_date: parseDate(g(first, "estimate_date")),
        status: g(first, "status") || "Draft",
        valid_until: g(first, "valid_until") || null,
        notes: g(first, "notes") || null,
        payment_terms: g(first, "payment_terms") || null,
        subject: g(first, "subject") || null,
        project_name: g(first, "project_name") || null,
  ...
  const mappedWithItems = useMemo(
    () => mappedEstimates.filter((e) => e.items.length > 0),
    [mappedEstimates]
  );
  ...
  const handleImport = async () => {
    ...
    startGlobalProcessing("Importing estimates…");
    const chunks: MappedEstimate[][] = [];
    ...
    try {
      for (let i = 0; i < chunks.length; i++) {
        ...
        const res = await importEstimatesFromCsv(companyId, chunks[i]!);
        if (res.error) {
          setResult(res);
      ...
      if (!cancelRequestedRef.current) {
        endGlobalProcessing({
          success: `Imported ${totalImported} estimate${totalImported !== 1 ? "s" : ""}.`,
        });
      }
      const totalSkipped =
        unlinkedCustomerNames.length + allSkippedNoCustomer.length + allSkippedDuplicateNumber.length;
      setResult({
  ...
  const reset = () => {
    ...
    setHeaders([]);
    setRows([]);
    setMapping({} as EstimateColumnMapping);
    setCustomerMapping({});
    setResult(null);
    setIsDragging(false);
    setImportProgress(0);
    ...
    <div className="flex h-full min-h-0 w-full flex-col">
      <div className="flex min-h-0 flex-1 flex-col overflow-auto">
        <p className="flex-shrink-0 px-4 py-2 text-sm text-[var(--color-on-surface-variant)]">
          {step === "upload" &&
            "Upload a CSV file (e.g. Zoho Quote export). You will map columns, then link each CSV customer to one in your database, then import."}
          {step === "mapping" &&
            "Map CSV columns to estimate and line item fields. Quote identifier is used to group rows into one estimate per quote."}
          {step === "map_customers" &&
            (distinctCustomerNames.length > 0
              ? `${linkedCustomerCount} of ${distinctCustomerNames.length} customers linked. Link each CSV name to a customer or choose "Don't import" to skip those estimates.`
              : `Link each customer name from the CSV to a customer in your database. Estimates for Don't import will be skipped.`)}
          {step === "preview" &&
            `${toImport.length} estimate${toImport.length !== 1 ? "s" : ""} will be imported. ${mappedWithItems.length - toImport.length} skipped (not linked in previous step).`}
        </p>
              ...
              <div className="flex min-h-0 flex-1 flex-col gap-4">
                <div className="min-h-0 flex-1 overflow-auto rounded-xl border border-[var(--color-outline)]">
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody>
                      {ESTIMATE_FIELDS.map(({ key, label, level }) => (
                        <tr
                          key={key}
                ...
                <p className="text-sm text-[var(--color-on-surface-variant)]">
                  ...
                  {rows.length >= MAX_ROWS && ` (max ${MAX_ROWS})`} — {grouped.length} quote group
                  {grouped.length !== 1 ? "s" : ""} (grouped by quote identifier).
                </p>
              </div>
            )}
              ...
              <div className="flex min-h-0 flex-1 flex-col gap-4">
                ...
                {distinctCustomerNames.length === 0 ? (
                  ...
                  <p className="text-sm text-[var(--color-on-surface-variant)]">
                    ...
                    Link each to a customer in your database or choose &quot;Don&apos;t import&quot; to skip those estimates.
                  </p>
                )}
              </div>
            )}
                ...
                {unlinkedCustomerNames.length > 0 && (
                  <div className="rounded-xl border border-[var(--color-outline)] bg-[var(--color-surface-variant)] p-3">
                    <p className="mb-2 text-sm font-medium text-[var(--color-on-surface)]">
                      Customers not linked (estimates will be skipped)
                    </p>
                    ...
                    <tbody>
                      {toImport.map((est, i) => (
                        <tr key={i} className="border-t border-[var(--color-divider)]">
                          ...
                          <td className="p-3 text-[var(--color-on-surface-variant)]">
                            {Number(est.total_amount).toFixed(2)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                  ...
                  <div className="rounded-xl border border-[var(--color-outline)] bg-[var(--color-surface-variant)]/30 p-4 space-y-3">
                    ...
                    <div className="grid gap-2 text-sm">
                      <p className="text-[var(--color-on-surface)]">
                        <span className="font-medium">{result.imported}</span> estimate
                        {result.imported !== 1 ? "s" : ""} linked to customers in our database and imported.
                      </p>
                      <p className="text-[var(--color-on-surface-variant)]">
                        ...
                        {result.skippedNoCustomer.length > 0 && (
                          <span> {result.skippedNoCustomer.length} no matching customer in database</span>
                        )}
                        {result.skippedNoCustomer.length > 0 && result.skippedDuplicateNumber.length > 0 && ", "}
                        {result.skippedDuplicateNumber.length > 0 && (
                          <span>{result.skippedDuplicateNumber.length} duplicate estimate number</span>
                        )}
                ...
                {importing && (
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm text-[var(--color-on-surface-variant)]">
                      <span>Importing…</span>
                      <span>
                        {importedCount} / {toImport.length} estimates imported ({importProgress}%)
                      </span>
                    </div>
                    <div className="h-2 w-full overflow-hidden rounded-full bg-[var(--color-surface-variant)]">
                      <div
