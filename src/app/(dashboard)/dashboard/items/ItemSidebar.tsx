...
import type { ItemSidebarItem } from "./ItemsDataContext";
...
export function ItemSidebar({
  ...
  const isNew = pathname === "/dashboard/items/new";
  const activeId =
    pathname.startsWith("/dashboard/items/") &&
    pathname !== "/dashboard/items" &&
    !isNew &&
    !pathname.startsWith("/dashboard/items/import")
      ? pathname.replace("/dashboard/items/", "").split("/")[0]
      : null;
  ...
  useEffect(() => {
    ...
    if (idx >= 0) {
      ...
      router.replace(qsClean ? `/dashboard/items?${qsClean}` : "/dashboard/items", { scroll: false });
    }
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      const msg = result.deletedCount === 1 ? "Item deleted." : "Items deleted.";
      if (result.skippedCount && result.skippedCount > 0) {
        endGlobalProcessing({ success: `${msg} ${result.skippedCount} skipped.` });
      } else {
        ...
        <div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto px-3 pb-3">
          {filtered.length === 0 ? (
            <div className="py-6 text-center text-sm text-[var(--color-on-surface-variant)]">
              {searchQueryProp?.trim() ? "No matches" : "No items yet"}
            </div>
          ) : (
            <div
              style={{
            ...
            >
              ...
              {virtualizer.getVirtualItems().map((virtualRow) => {
                ...
                const content = (
                  <>
                    <div
                      className={`truncate text-sm font-medium ${selectionMode ? "flex-1 min-w-0" : ""} text-[var(--color-card-text)]`}
                    >
                      {isActive ? (
                        <span className="text-[var(--color-on-primary-container)]">{c.name || "—"}</span>
                      ) : (
                        c.name || "—"
                      )}
                    </div>
                ...
                return (
                  ...
                  >
                    {selectionMode ? (
                      <Link
                        href={`/dashboard/items/${c.id}${itemDetailQuery()}`}
                        scroll={false}
      ...
      <ConfirmDialog
        ...
        message={
          selectedIds && selectedIds.size === 1
            ...
            : `Delete ${selectedIds?.size ?? 0} items?`
        }
        confirmLabel="Delete"
        variant="danger"
