...
export function Drawer({
  ...
  return (
    <Dialog.Root open={open} onOpenChange={(open) => !open && onClose()}>
      {/* Only mount portal when open to avoid Radix DOM cleanup touching removed nodes (parentNode null) */}
      {open && (
        <Dialog.Portal>
          <Dialog.Overlay
          ...
          >
            {/* Dialog.Title must be present for a11y; Radix may require it as first child */}
            <Dialog.Title
